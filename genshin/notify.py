...
class Notify(object):
    ...
    def __init__(self):
        ...
        self.PUSH_CONFIG = os.environ.get('PUSH_CONFIG')
        self.DISCORD_WEBHOOK = os.environ.get('DISCORD_WEBHOOK')
        self.TELEGRAM_BOT_TOKEN = os.environ.get('TELEGRAM_BOT_TOKEN')
        self.TELEGRAM_BOT_CHAT_ID = os.environ.get('TELEGRAM_BOT_CHAT_ID')
    ...
    def telegramBotMessage(self, message):
        bot = telegram.Bot(self.TELEGRAM_BOT_TOKEN)
        if not self.TELEGRAM_BOT_CHAT_ID:
            updates = bot.get_updates()
            print(self.TELEGRAM_BOT_CHAT_ID) 
            if len(updates) == 0:
                raise Exception("Telegram bot has no chats. Please message the bot before running this application")
            self.TELEGRAM_BOT_CHAT_ID = updates[0].message.chat.id
            print(self.TELEGRAM_BOT_CHAT_ID) 
            dotenv.set_key(self.dotenv_file, "TELEGRAM_BOT_CHAT_ID", str(self.TELEGRAM_BOT_CHAT_ID))
           
        bot.send_message(text=message, chat_id=self.TELEGRAM_BOT_CHAT_ID)
    ...
    def send(self, app='Genshin Daily Sign-In', status='', msg='', **kwargs):
        ...
        if self.PUSH_CONFIG or self.DISCORD_WEBHOOK:
            ...
            self.telegramBotMessage(status + "\r\n" + msg)
            sent = True
        if not sent:
